...
import {
  ...
  trainingEffectLabel, formatDuration, formatSpeed,
  formatHr, formatTrainingEffect,
} from '../../lib/athletics'
...
function Row({ label, value, subValue, info }) {
  return (
    <div className="flex justify-between items-center px-4 py-3" style={{ borderBottom: '1px solid rgba(255,255,255,0.04)' }}>
      <span className="text-xs flex items-center gap-1.5" style={{ color: 'var(--color-slate)' }}>
        {label}
        {info}
      </span>
...
// Dati atletici — interamente nascosta se il match non ne ha (i match
// registrati prima di questa feature non hanno il campo `athletics`).
function AthleticsSection({ athletics }) {
    ...
    <div className="mb-4">
      ...
      {rows.length > 0 && (
        <div className="rounded-2xl overflow-hidden mb-2" style={{ background: 'var(--color-surface-2)' }}>
          {rows.map(r => <Row key={r.key} label={r.label} value={r.value} subValue={r.subValue} info={r.info} />)}
        </div>
      )}
...
// Le due soglie di colore (5 segmenti pieni + il cap rosso a 5,0) ricalcano
